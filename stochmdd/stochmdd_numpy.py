...
def MDDminibatch(nt, nr, dt, dr, Gfft, d, optimizer, n_epochs, batch_size,
    ...
    # Define source order (to be kept fixed when shuffle=False
    isrcs = np.arange(ns)
    if shuffleonce:
    ...
    for epoch in range(n_epochs):
        ...
        for ibatch in range(int(np.ceil(ns / batch_size))):
            ...
            # Update model
            optimizer.step(grad.reshape(model.shape), ibatch=ibatch)
...
def MDDminibatch_repetition(nt, nr, dt, dr, Gfft, d, optimizer, n_epochs, batch_size,
                            twosided=True, mtrue=None, ivstrue=None, enormabsscaling=False,
                            seed=None, scheduler=None, epochprint=10, reciprocity=False,
                            savegradnorm=False, savefirstgrad=False, timeit=True,
                            kwargs_sched=None, **kwargs_solver):
    r"""MDD with mini-batch stochastic average gradient descent methods

    These methods require storing the past gradients and can select batches
    ...
    isrcs = np.arange(ns)
    np.random.shuffle(isrcs)
    batches = [isrcs[ibatch * batch_size:(ibatch + 1) * batch_size]
               for ibatch in range(int(np.ceil(ns / batch_size)))]
    nbatches = len(batches)
    if len(batches[-1]) == 1:
        raise NotImplementedError('Last batch has size equal 1... this is '
                                  'currently not implemented, ensure it '
    ...
    for itr in range(n_epochs * nbatches):
        # Choose batch
        ibatch = np.random.randint(0, nbatches)
        isrcbatch = batches[ibatch]
        MDCop.update(isrcbatch)
        ...
        # Update model
        optimizer.step1(grad.reshape(model.shape), ibatch=ibatch)
        ...
        if (itr + 1) % (epochprint*nbatches) == 0:
            print(f'itr: {itr + 1:3d}, loss : {loss.item() / ((2 * nt - 1) * batch_size):.4e}')
    ...
    if not savefirstgrad:
        return model, data, losshist, enormhist, lr
    else:
        return model, data, losshist, enormhist, gradfirst
